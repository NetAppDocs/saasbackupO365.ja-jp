...
個々の Microsoft Exchange Online メールボックスアカウントのユーザーは、 SaaS Backup for Microsoft 365 のセルフサービスポータル（ SSP ）を使用して、 Microsoft Exchange Online のアイテムを検索してリストアできるようになりました。セルフサービスポータルを有効にするには、管理者に問い合わせてください。

* テナントに対して Advanced Search * を有効にする必要があります。メールボックスアイテムのインデックスを作成するには、フルバックアップが必要です。

* link:task_using_advanced_search.html["高度な検索を使用します"]
* link:reference_about_ssp.html["セルフサービスポータルについて"]
* link:task_use_ssp.html["セルフサービスポータルを使用する"]
